...
def show_json():
    ...
    train_time_index = st.selectbox("制限時間(train_time_index)", options=number_options, key="train_time_index")
    colors = st.selectbox("色(colors)", options=number_options, key="colors")
    start_delay_index = st.selectbox("スタート遅延(start_delay_index)", options=delay_options, key="start_delay_index")

    st.markdown("## D.ロジック設定")
    logic_index_g = []
    for i in range(4):
        logic_index_g.append(st.selectbox(f"xxx(logic_index_g[{i}])", options=number_options, key=f"logic_index_g_{i}"))
